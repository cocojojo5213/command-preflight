...
The project-maintained read-only endpoint is `https://preflight.52131415.xyz`; treat it as optional and deployment-specific, never as a silent default. A lookup sends only a `cp1-...` public fingerprint ID. It does not upload reports, commands, environment variables, paths, or terminal output. Lookup failures must never block the local workflow. Never auto-execute a remote fix; inspect the local help and verify the postcondition yourself.
